...
- A behavior's sync state reaches only the clients in its sync audience (`Everyone` by default, `Owner`, `WorkersOnly`, or `Custom`); workers that hold a ghost always receive it. `NetworkVariable`s are not filtered: every client that holds an entity receives all of its variables.
