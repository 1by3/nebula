...
- A client reconnects by itself with its session token, through any gateway of the mesh, and keeps its pawn when it returns within `SessionReclaimSeconds`. This also covers a gateway that is lost without a clean shutdown (killed, crashed): session coordination treats a gateway whose control-plane heartbeat has gone stale as gone and reclaims its sessions on another gateway, bounded in practice to single-digit seconds (see `docs/gateway-fleet-audit.md` and the gateway-kill rows of `docs/scale-suite.md`). Nebula does not provide the load balancer that spreads clients over several gateways or redirects a client's reconnect to a live one, and the orchestrator starts only its own gateway and does not restart it.
