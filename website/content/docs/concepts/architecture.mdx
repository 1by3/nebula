...
This page describes the Nebula architecture: the model the runtime is built on and the direction it grows in. The [roles and wire protocol](/docs/concepts/roles-and-wire-protocol) page and the guides describe the running code in detail.
...
The claim path is `[AuthorityRpc]`; see [RPCs](/docs/guides/rpcs).
