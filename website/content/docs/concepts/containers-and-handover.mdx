...
The [architecture](/docs/concepts/architecture) describes containers, seams and handover as designed. This page describes what the prototype actually does today, which is a smaller mechanism with the same shape: authority per container, pre-warm through a ghost band, flip with a handed state and a new epoch.

## Containers

A `Container` is a MonoBehaviour on a scene object: a box volume (`Size`, centred on `Center`) with a stable `ContainerId`. The object's transform defines the container's local coordinate space. Entities inside a container are parented under its transform, and every position on the wire is expressed in container-local space, so a container could move without its contents ever changing coordinates. The prototype's containers are static.
...
There are no seam volumes, no `Seam` component and no bake validator in the prototype. Instead every container boundary carries two automatic parameters from `NebulaConfig`:
...
4. Maintains the ghost band: any authoritative entity within `GhostBandMargin` of a neighbouring container owned by another worker gets a `GhostSpawn` on that worker, then `GhostState` at 60 Hz, `GhostVars` when a NetworkVariable changes and `GhostSyncState` for the sync channel.
