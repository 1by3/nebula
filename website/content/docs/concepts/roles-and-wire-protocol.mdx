...
No encryption or authentication on any link, no interest management, no delta compression or quantisation (every var change ships the whole blob; positions are full floats), and no NAT traversal. The lateral link is meant for a private datacenter network; the client link works on the open internet but is not hardened for it.
