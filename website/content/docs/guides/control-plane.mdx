...
| `draining` | Reserved. Not produced today; retirement reuses the per-entity handover path with the lease staying `active` until it moves. |
...
| `ready` | Simulating. The only status the worker's heartbeat reports today. |
| `draining` | Reserved for a worker that is handing everything over. Not reported today; the orchestrator tracks retirement on its own side. |
...
<Cards>
  <Card title="Orchestrator and dashboard" href="/docs/guides/orchestrator-and-dashboard" description="Who writes the leases and when." />
