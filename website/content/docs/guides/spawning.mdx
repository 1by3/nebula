...
- **No mesh singleton.** There is no way to elect one worker to run a piece of logic (one boss, load-based placement). Anything that needs a single decision-maker has to be expressed as a reconcile against a shared value, or wait for that feature.
- **No persistence.** Entities live only on workers. A worker that dies takes its authoritative entities with it; the gateway drops them and players are respawned elsewhere through `OnSpawnPlayer`. Settings persist only as long as the control plane's database does.
- **No client-side spawning.** Only workers spawn. A client that wants something created sends a `ServerRpc` and the owning worker spawns it (see [RPCs](/docs/guides/rpcs)).
