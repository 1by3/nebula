...
A brain perceives what its process holds. On a worker that is its own pawns plus the ghosts of its neighbours' pawns within the ghost band, so an NPC standing well inside a building another worker owns would never notice a player fifteen metres outside the door, while that player can hit it (see [shots that cross workers](#shots-that-cross-workers)). The sample's `PresenceRelay` closes that gap without ghosting anything: every 15 ticks each worker sends each connected neighbour one `PresenceQuery` ([a worker query](/docs/guides/rpcs#queries)) with the eye position of every live NPC it drives, and the neighbour answers with the live pawns it owns within `NpcSightRange` of each eye, the eight nearest per NPC, as position, velocity, net id and name. The brain's `FindVisibleEnemy` considers those snapshots alongside `PlayerController.All`, with the same view cone and `Physics.Linecast` line-of-sight test (a worker has its neighbours' cells loaded, `WorkerLoadRingCells`). A pawn this worker also holds as a ghost is taken as the ghost, never as its snapshot.

The brain aims at an `EnemyTarget`: a local pawn, or a snapshot whose position is carried along its velocity since the report (half a second at most). It only fires at a snapshot its owner refreshed within the last two report periods, and drops a target no report has mentioned for two seconds. The shot itself goes through `ShotRelay` unchanged, which asks the owners of the containers along the ray. A reply that lands after the NPC was handed over is discarded, so nothing is aimed on behalf of a pawn this worker no longer drives.
...
private void ServerFire(uint tick, uint aimTick)
{
    ...
    ShotRelay.Instance.Fire(this, tick, rewindTick, EyePosition, AimDirection, Range, Damage);
}
```

```csharp title="Hitscan.cs"
public static float LevelDistance(Vector3 origin, Vector3 dir, float range)
{
    return Physics.Raycast(origin, dir, out var hit, range, PlayerController.LevelMask, QueryTriggerInteraction.Ignore) ? hit.distance : range;
}

public static PlayerController NearestPawn(List<PlayerController> pawns, Vector3 origin, Vector3 dir, ref float maxT, PlayerController exclude, uint rewindTick, bool authoritativeOnly)
{
    PlayerController victim = null;
    foreach (var pc in pawns)
    {
        if (pc == exclude || !pc.IsSpawned || pc.IsDead.Value) continue;
        if (authoritativeOnly && !pc.HasAuthority) continue;
        Vector3 pos;
        if (rewindTick != 0) pc.Identity.TryGetPoseAt(rewindTick, out pos, out _);   // falls back to the current pose
        else pos = pc.transform.position;
        var a = pos + Vector3.up * pc.Radius;
        var b = pos + Vector3.up * (PlayerController.CapsuleHeight - pc.Radius);
        if (RayHitsCapsule(origin, dir, a, b, pc.Radius, maxT, out float t)) { maxT = t; victim = pc; }
    }
    return victim;
}
```

The capsule test is pure geometry against the historical pose, so it never touches PhysX. The same two functions run on the owning client for the predicted bolt, with `rewindTick` 0 (the pawns as drawn), so the bolt stops where the worker will say it stopped. The 0.5 s cap on the rewind is the sample's policy, not Nebula's: it bounds how far into the past a laggy or malicious client can claim to have seen.

### Shots that cross workers

A worker only holds ghosts of its neighbours' entities within the ghost band, so a shooter outdoors could never hit someone ten metres inside a building another worker owns: on the shooter's worker that pawn does not exist. The sample's `ShotRelay` carries the shot instead of the entities:

1. The shooter's worker tests the level and its own authoritative pawns (never ghosts: their owner has better history).
2. `ContainerRegistry.Along(origin, end, margin, result)` lists the containers the ray crosses up to the nearest thing found; every other worker owning one of them gets a `ShotQuery` (a [worker query](/docs/guides/rpcs#queries)) with the ray, the aim tick and the distance to beat.
3. Each of them tests its own authoritative pawns rewound to the aim tick, applies the damage if it finds one nearer, and replies with a `ShotResult`.
4. When every reply is in (or the query's 300 ms timeout has passed), the shooter's worker hurts its own candidate unless a neighbour's pawn was nearer, draws the bolt for the clients, and credits the shooter with `CreditShot`, an `AuthorityRpc` that follows the shooter through a handover. A shot that needs no neighbour completes inside the same call.

The victim's worker applies damage as-is; there is no second validation on the receiving side. The trigger pull itself is made robust separately: `ShooterInput.FireSeq` counts pulls, so a pull whose tick arrived late (see [when input is missing](#when-input-is-missing-on-the-worker)) still fires on the first input the worker does consume rather than being dropped with the tick.
