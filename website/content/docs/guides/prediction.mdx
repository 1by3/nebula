...
2. `ContainerRegistry.Along(origin, end, margin, result)` lists the containers the ray crosses up to the nearest thing found; every other worker owning one of them gets a `ShotQuery` [worker message](/docs/guides/rpcs#worker-messages) with the ray, the aim tick and the distance to beat.
...
4. When every reply is in (or 300 ms have passed), the shooter's worker hurts its own candidate unless a neighbour's pawn was nearer, draws the bolt for the clients, and credits the shooter with `CreditShot`, an `AuthorityRpc` that follows the shooter through a handover.
