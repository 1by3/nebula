...
Binding is reflection-based today (`RpcRegistry`); a source generator could replace it without changing gameplay code.
...
The victim's worker applies the claim as-is: there is no rewind or validation of the claim on the receiving side today. The shooter's worker is trusted, which is fine because every worker is in the same trusted datacenter network.
