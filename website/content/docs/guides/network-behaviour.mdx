...
A `NetworkVariable<string>` that changes often re-sends every other variable on the entity with it. This is fine for a prototype and is the kind of thing a source generator with per-field dirty bits would fix; it is not implemented yet.
</Callout>
