...
- The identity stream sends the root pose and velocity every tick.
...
- The **sync channel** sends component data by tick. `NetworkTransform` and `NetworkAnimator` use this channel.
...
Add [`NetworkRigidbody`](/docs/reference/core/network-rigidbody) to a physics entity controlled by a worker. Add it to the same GameObject as its `NetworkIdentity` and Unity `Rigidbody`.
...
When a worker gains authority, `NetworkRigidbody` makes the body dynamic and restores its linear and angular velocity. When the worker loses authority, the component makes the body kinematic.
...
Call `SetVelocity(Vector3 linear, Vector3 angular = default)` on the authoritative worker to launch a newly spawned rigidbody. The method updates `Identity.Velocity` and the dynamic `Rigidbody`:
