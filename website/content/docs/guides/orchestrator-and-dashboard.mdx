---
title: Orchestrator, Nebula Dashboard and worker hosts
description: How the orchestrator keeps workers running and containers assigned, what the Nebula Dashboard and its HTTP API expose, and how worker hosts decide where a worker runs.
---
...
## The Nebula Dashboard

The orchestrator serves the Nebula Dashboard, a single page from `Resources/NebulaDashboard.html`, on `DashboardPort` (default 7080, `-nebula-dashboard-port`; 0 disables it). It binds to `localhost` unless told otherwise:
...
| `-nebula-telemetry <url>` | Where launched workers post [World map](#world-map) telemetry. Defaults to `http://<advertise address>:<dashboard port>/api/telemetry`; `off` disables it. |

The dashboard has two pages, **Overview** (`/`) and [**World map**](#world-map) (`/map`). The overview polls `/api/state` and shows:
...
- **Carried containers**: the interiors of ships and other vehicles ([dynamic containers](/docs/guides/dynamic-containers)), whose leases workers create at runtime. Each follows its carrier's worker unless you pin it to a worker of its own from the row's drop-down (`POST /api/containers/pin` with `{"containerId": "starhopper#…", "workerId": "w2"}`; `/api/containers/unpin` reverts). A pinned interior falls back to following its carrier when its worker dies or retires.
- **Gateways**: id, address, heartbeat age.
- **Events**: the orchestrator's last 60 log lines (it keeps 200), colour-coded by level.

### World map

`/map` draws the mesh from above: the cells of a [partitioned world](/docs/guides/world-partition), every static container in absolute world coordinates filled with the colour of the worker that owns it, the [carried containers](/docs/guides/dynamic-containers) moving where their carriers are, and the entities the workers simulate. Drag to pan, scroll to zoom, hover or click anything for the inspector:

- **A container**: owner, lease and epoch, its box, the containers it sits in and contains, the ones it has seams with, and how many players, bots, server-driven and other entities it holds, per worker, with the ghosts other workers keep of them. A cell container also lists the workers that have the cell loaded.
- **A carried container**: which worker simulates the hull and which one the interior (they differ when it is pinned), the lease, speed, heading and the container it is in, with **Follow** (the camera tracks it) and the same **Pin interior to…** / **Unpin** as the overview.
- **An entity**: kind, authoritative worker, container, position and heading.

The rail switches the fill between owner and entity load, toggles layers (cells, containers, carried, entities, the ghost band drawn `GhostBandMargin` inside each wall, labels), and lists workers (hover one to light up what it owns, click to keep it lit) and carried containers.

The live half comes from the workers. Each posts a telemetry document to `POST /api/telemetry` ([`WorkerTelemetry`](/docs/reference/worker/worker-telemetry)) once a second: per container, how many players, bots, server-driven and other entities it is authoritative for and how many ghosts it holds; the box, pose and velocity of every carried container whose carrier it simulates; and the cells it has loaded. While somebody has the map open (the page read `/api/map` in the last 5 s) the reply asks for more, and the worker posts every 0.5 s with one point per authoritative entity, at most 4096. Positions are absolute: every process adds its [floating origin](/docs/guides/world-partition) back, so workers with different origins agree. The document is built in `Update`, never inside the tick, and posted from the thread pool; a post still in flight skips the next one.

The orchestrator passes `-nebula-telemetry http://<advertise address>:<dashboard port>/api/telemetry` to every worker it launches (the address worker VMs already download the build from), and a worker in the orchestrator's own process hands its documents over directly. A worker started by hand needs the switch itself; `-nebula-telemetry off` turns telemetry off. The orchestrator keeps each worker's latest document ([`MeshTelemetry`](/docs/reference/orchestrator/mesh-telemetry)), drops it when the worker dies, retires or stops posting for 15 s, and serves it unparsed; the page joins it with `/api/state`. With the dashboard disabled there is no map and no telemetry.
...
Reads are answered from the listener thread with the last published state, and so is worker telemetry (`/api/map`, `/api/map/geometry` and `/api/telemetry` touch nothing Unity owns). Other writes are queued and executed on the orchestrator's main thread; the HTTP response waits up to 3 s for that and returns `503` otherwise. Every successful POST answers `{"ok":true,"desired":n}`; errors answer `{"ok":false,"error":"..."}`.
...
| `POST /api/containers/pin` | `{"containerId": "starhopper#…", "workerId": "w2"}` | Give a carried container a worker of its own. `400` for a static container, an unknown one or a worker that is not live. |
| `POST /api/containers/unpin` | `{"containerId": "starhopper#…"}` | Let a pinned carried container follow its carrier again. |
| `GET /map` | none | The World map page. |
| `GET /api/map/geometry` | none | The map's static half, built at startup: `partitioned`, `world` (`name`, `cellSize`, `cells[]` with `coord` and `scene`), `ghostBandMargin`, `handoverHysteresis` and `containers[]` with `id`, `index`, `cell`, `isCell`, `center`, `size`, `rotation` (quaternion), `parent` (smallest enclosing container) and `neighbors`. |
| `GET /api/map` | none | `geometryVersion` and `workers[]`: each worker's latest telemetry document (`doc`) with its `ageSeconds`. Reading it asks workers for entity positions for the next 5 s. |
| `POST /api/telemetry` | a worker's telemetry document | Store it. Answers `{"ok":true,"detail":true\|false}`: whether the worker should include entities next time. `400` for a document that is not a JSON object starting with `"worker"`, `413` above 8 MB. |
