...
| `deploy.*` | Defaults for `nebula deploy`: the target (`hetzner` is the only one today), the mesh name used to label cloud resources, the Maincloud database, worker and NPC counts. Optional `workerType`, `orchestratorType` and `location` override the VM types and region from the CLI config; `ccx13` gives dedicated vCPUs when measuring tick time. |
