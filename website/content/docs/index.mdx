---
title: Introduction
description: What Nebula is, what it is not yet, and where to go next.
---
...
This site documents the prototype in the `nebula` repository. It is a complete, working end-to-end mesh, deliberately small: four containers, N workers, one gateway, one orchestrator, one control plane, and a first-person shooter that exists to make crossing a boundary mid-firefight visible.
...
Nebula is a prototype, and the docs say so wherever it matters. There is no persistence, no rewind on hit validation, no delta compression, no authored seam volumes, no warm pool of workers, and the control-plane reducers accept any caller. What has been verified on one machine and on Hetzner Cloud VMs is listed on the [architecture](/docs/concepts/architecture#what-the-prototype-proves) page, and every guide has a section on what is deliberately not there.
