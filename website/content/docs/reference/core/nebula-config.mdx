...
Where entities carrying a PersistentEntity are stored: 'auto' (local when the control plane is local, otherwise spacetime), 'spacetime', 'local' (a file next to the process) or 'off'. -nebula-persistence-mode overrides.

### PersistenceUri [#persistenceuri]

```csharp
public string PersistenceUri = "";
```

SpacetimeDB instance holding the persistence database. Empty = the same one as the control plane. -nebula-persistence overrides.
...
Persistence database name; a database of its own next to the control plane. -nebula-persistence-database overrides.
