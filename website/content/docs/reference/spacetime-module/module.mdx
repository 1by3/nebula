---
title: "Module"
description: "Defines the SpacetimeDB tables and reducers used by the Nebula control plane."
---
...
Defines the SpacetimeDB tables and reducers used by the Nebula control plane.
...
```csharp
[SpacetimeDB.Reducer]
public static void EnsureRuntimeContainer(ReducerContext ctx, string containerId, string workerId, float centerX, float centerY, float centerZ, float sizeX, float sizeY, float sizeZ);
```

Make sure a lease row exists for a runtime container, carrying its box and, when workerId is not empty, already assigned to that worker (epoch 1, active). A no-op when the row exists: the first worker to ask owns the container, the second sees the row.
...
A worker still wants a runtime container it does not own (an entity of its is next to it): stamp the row so the owner knows the box is in use mesh-wide and does not retire it.
...
Orchestrator: authoritatively assign a container to a worker. Bumps the container epoch so stale owners can be rejected. A pinned row is left alone: only PinContainer moves it and SetLeaseState releases it, so a carrier's worker reclaiming the lease with a call already in flight cannot undo a pin.
...
Orchestrator: give a carried container a worker of its own. Worker, epoch and the pinned state change in one transaction; as two calls (assign, then set the state) the carrier's worker sees the in-between "active" row, reclaims it, and wins.
...
Small dense index handed out by the orchestrator. Used for entity id allocation (high bits) and debug colors.
...
Runtime containers (registered by the game while the mesh runs) carry their box here, in absolute world coordinates, so every process registers the same box from the row alone. False for a baked or carried container.
