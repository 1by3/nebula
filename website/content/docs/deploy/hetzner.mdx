...
Run `nebula deploy` to deploy Nebula to Hetzner Cloud. The command publishes the control plane to SpacetimeDB, creates one virtual machine (VM) for the orchestrator and gateway, and lets the orchestrator create one VM per worker.
...
Nebula.x86_64 -batchmode -nographics -nebula-role orchestrator -nebula-host hetzner
  ...
  -nebula-spacetime https://maincloud.spacetimedb.com -nebula-database my-game
...
`destroy` lists the servers labelled with the mesh, asks for confirmation (`--yes` skips it) and deletes them all, workers and orchestrator. `--all` also deletes the firewalls, the private network and the ssh key after a short wait for the servers to detach. Once the servers are gone it runs `spacetime delete` for `deploy.database` and `deploy.persistenceDatabase` on the configured Spacetime server, which removes every saved entity; `--keep-data` keeps both databases. The ssh key file in `~/.ssh` is left alone.
