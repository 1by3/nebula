...
The Nebula Dashboard at `http://<public ip>:7080/` works exactly as it does locally: copy the gateway address for your client, change the desired count, add, remove or kill workers, edit settings such as `npcs`. Each change creates or deletes real VMs. See [Orchestrator, Nebula Dashboard and worker hosts](/docs/guides/orchestrator-and-dashboard).
