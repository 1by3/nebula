...
- **Unauthenticated control plane.** The module's reducers accept any caller, so anyone who knows the Maincloud database name can register a fake worker or rewrite leases. A shared secret checked inside the reducers is the next step.
- **Unauthenticated dashboard** on a public port. Anyone who finds `tcp/7080` can resize or kill your mesh. Restrict the orchestrator firewall's dashboard rule to your own IP if that matters before it is fixed.
- **No warm pool.** A replacement worker is a fresh VM about 30 s away, not an instant failover. Its transient entities are lost; players respawn.
- **No rolling redeploy.** Every `nebula deploy` restarts the orchestrator and recreates every worker.
- **One mesh per Hetzner project**, named by `meshName`.
