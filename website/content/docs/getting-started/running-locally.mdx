...
The Nebula Dashboard at `http://localhost:7080/` shows the same and more: the gateway address clients connect to, with a copy button; every worker with its authoritative and ghost entity counts, tick time and heartbeat age; the container-to-worker table with lease epochs; mesh settings; and the event log. Its buttons add a worker, remove one gracefully, kill one (a simulated crash; it is relaunched) and force a rebalance. The same operations are an HTTP API, described in [Orchestrator and Nebula Dashboard](/docs/guides/orchestrator-and-dashboard).
