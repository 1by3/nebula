...
- **A worker dealt hundreds of containers at once could be declared dead, and take the next worker with it.** When a worker gained a batch of leases (a re-deal after another worker died, a rebalance along a boundary, a burst of runtime chunks), it sent one persistence load per container to the orchestrator, all at the same time, each on the thread pool. In a Unity (Mono) player a few hundred of them starved the thread pool: no load completed, and neither did the control-plane long poll or the heartbeat, which share that pool and the same orchestrator address. After `WorkerTimeoutSeconds` the orchestrator declared the worker dead and dealt its containers to the next worker, which received the same burst. A sample with fast ships and a runtime chunk grid lost every worker this way. Restores and every other persistence read are now bounded.
  ...
  - Conformance scenario 16, `ConformanceRestoreBurstTests` (Unity tier for the worker, .NET tier for the stores, the endpoint and the in-flight bound), covers the fix.
