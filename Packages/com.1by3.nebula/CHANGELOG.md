...
Wire protocol 21. The change is additive for clients, and the window moves to 20..21: a gateway admits clients of protocol 20 and 21 (`MinProtocolVersion` is now 20, so protocol-19 clients are refused). Gateways and workers of a mesh must still run the same build.

### Added

- **Replicated maps that send only what changed.** `NetworkMap<TKey, TValue>` is a keyed collection declared on a `NetworkBehaviour` like a `NetworkVariable`. The authoritative worker writes it (`this[key] = value`, `Add`, `TrySet`, `Remove`, `Clear`, `SetDirty`) and every copy reads it as an `IReadOnlyDictionary`. Each tick, only the keys that changed are sent, each with its final value (`EntityMaps` to gateways, `GhostMaps` to ghost workers), and a variable change no longer has anything to do with it: maps are left out of the variable block. The gateway keeps each map's current contents, so a client that starts holding the entity later gets one full copy in its spawn and then the same changes as everyone else. A handover carries the whole map, including changes not sent yet. `OnChanged` raises one `NetworkMapChange` per key (`Added`, `Updated`, `Removed`) on every copy, and a full copy received on top of existing contents is diffed. `[Persist]` saves a map like a variable. A map is capped at 60 KB encoded by default (`capacityBytes` on the constructor). New: `NetworkMap<TKey, TValue>`, `NetworkMapBase`, `NetworkMapChange<TKey, TValue>`, `NetworkMapChangeKind`, `NetworkMapCodec`, `NetworkMapCache`, `EntityMapsMsg`, `MsgId.EntityMaps` (38), `MsgId.GhostMaps` (50), `EntitySpawnMsg.Maps`, `NetworkIdentity.HasMaps`, `WriteMapsFull`, `WriteMapsDelta`, `ReadMaps`. See the replicated maps guide and `docs/replicated-collections.md`. `NetworkMapTests` and conformance scenario 29 cover it, in both tiers. (NEB-335)
- **Chunk state sends only the entries that change.** `ChunkStateEntity` now replicates its entries through a `NetworkMap`, so mining one rock sends that entry instead of the chunk's whole map (up to 48 KB). The public API, events, limits and the saved record format are unchanged, so existing records load. (NEB-335)

- **Retire a scope that still holds saved objects or NPCs your game rebuilds.** The default retire rule never retired a scope holding any entity, so one saved crate kept a world running forever, and replacing `ScopeLifecycle.ShouldRetire` had no effect on the standalone orchestrator that `nebula start` and a cloud deployment run, which runs no game code. `NebulaConfig.ScopeRetireWithEntities` (`-nebula-scope-retire-with-entities`, off by default) makes the default rule retire an idle scope that no client-owned entity is in: its persistent entities are checkpointed and come back with it. It is exported to the standalone services like the other settings and reaches a policy as `ScopeRetireContext.RetireWithEntities`. A worker also kept a part busy while it held any non-persistent entity, so an NPC a game respawns from its own state held the scope's idle clock at zero; `NebulaLifecycle.DiscardOnRetire` names the entities a retire may throw away, and they no longer keep their part busy. Client-owned entities and unsaved persistent changes still do. Default behavior is unchanged. See "Retire a world that still holds things" in *Simulation scopes*. (NEB-339)
...
- **Ghost a large entity by its extent.** The ghost band measured every entity from its root position, so a building, a player-built structure or a large ship whose root was away from a seam, but whose walls reached across it, had no ghost on the neighboring worker, and a player simulated there walked through it. An entity can now declare an extent on its `NetworkIdentity` (**Ghost band** in the inspector): `EntityExtentSource.Explicit` with a box in the entity's local space, or `EntityExtentSource.Colliders`, a box around its own enabled, non-trigger colliders that is computed again every `ColliderExtentRefreshTicks` (60) ticks, when a direct child changes, and after `RefreshExtent()`. The ghost band then measures the box instead of the root and ghosts the entity to every worker whose container it comes within `GhostBandMargin` of, including containers that do not touch its own. Authority, handover, lingering, carrier contents and cohesion groups are unchanged, and an entity without an extent (the default, `None`) is measured by its root at no extra cost. The extent works in a physics frame's own coordinates, so a structure on a planet is measured against the planet's chunks. Change it at runtime with `SetExtent`, `UseColliderExtent` or `ClearExtent` on the authority, after a structure is edited for example: a runtime extent travels with a handover and is saved by `PersistentEntity`. New: `EntityExtentSource`, `NetworkIdentity.ExtentSource`, `Extent`, `TryGetExtent`, `SetExtent`, `UseColliderExtent`, `ClearExtent`, `RefreshExtent`, `ExtentChangedAtRuntime`, `ColliderExtentRefreshTicks`. See "Ghost a large entity by its extent" in *Containers and handover*, and `docs/entity-extents.md`. Conformance scenario 28 (`ConformanceEntityExtentTests`) and `EntityExtentTests` cover it. (NEB-334)
- Wire, additive: `AuthorityTransfer` gains a trailing optional extent section after the session section (`CarriesExtent`, `ExtentSource`, `ExtentCenter`, `ExtentSize`), written only for an entity whose extent was set at runtime. The protocol stays 20, and every worker of a mesh must still run the same build. `PersistentEntity` writes a persistence chunk of its own for such an entity; other records are unchanged. (NEB-334)

- **Build a Linux player in batch mode.** `NebulaBuild.BuildLinuxBatch` builds the Linux client (`StandaloneLinux64`, player subtarget) into `Builds/Linux64Player/Nebula.x86_64`, so a CI job can build every client target in batch mode, for example with GameCI on Linux runners. The folder is apart from the dedicated server's `Builds/Linux64`, so neither build overwrites the other. **Nebula > Build > Linux Player (Mono, development)** builds it from the Editor. New: `NebulaBuild.BuildLinux`, `BuildLinuxBatch`, `LinuxPlayerBuildDir`, `LinuxPlayerExecutablePath`.

### Fixed

- **An `InstanceBoundary` in a scoped grid never let anyone in.** A game that builds content per chunk in a scoped grid (a planet) could place a boundary in that content, but the boundary assumed the world around it was the public world: it ignored every entity outside scope 0, sent an occupant who left into the public world, compared positions across origin frames, and prepared the instance at the host frame's numbers, so a later origin shift made `PrepareInstance` throw from inside the worker tick. A boundary now stands in the scope of the container it is parented under (`HostInstanceId`; the public world for a boundary in the shared scene, as before). It considers only entities in that scope or in its own instance, compares positions in absolute coordinates, prepares the instance at its absolute position rounded to a millimetre (`InstanceOrigin`), and returns an occupant to a container of the host scope that holds the point, holding them inside until one is leased. A held position is kept in absolute coordinates, so an origin shift no longer moves a waiting player, and a preparation the control plane refuses is logged once instead of throwing. `ObservePublic` still observes the public world, so the boundary warns when it is on in a scoped grid. New: `InstanceBoundary.HostInstanceId`, `AbsolutePosition`, `InstanceOrigin`, `OriginResolution`, and `NebulaWorker.PrepareInstance(InstanceTemplate, string, Double3)`, which takes an absolute origin. See "Place a boundary in a scoped grid" in the instancing guide. `InstanceBoundaryScopedGridTests` covers it. (NEB-340)
...
- **Private sync state: choose which clients receive a behaviour's sync state.** Override `NetworkBehaviour.SyncAudience` with `Everyone` (the default, unchanged and free), `Owner` (only the client that owns the entity), `WorkersOnly` (no client), or `Custom`. For `Custom`, override `IsInSyncAudience(clientId, pawn)`: the authoritative worker asks it about every client whose pawn it holds when the entity gains authority, every `SyncAudienceRefreshTicks` ticks (30 by default, 0 for never) and at the next tick after `MarkSyncAudienceDirty()`. An audience holds at most 256 clients. Workers holding a ghost always receive every behaviour's state; an audience only restricts clients. The gateway applies the audience to every spawn, delta and keyframe it sends and to the cached keyframes a late joiner starts from, so a client outside an audience never receives the state's bytes. A client that joins an audience gets the newest keyframe at once and a fresh one in the same tick; a client that leaves it gets `NetworkBehaviour.OnSyncStateCleared()`. The owner keeps receiving `Owner` state across a handover and a reconnect with its session token. `NetworkVariable`s are not covered: they still go to every client that holds the entity. New: `SyncAudience`, `NetworkIdentity.SyncAudienceGeneration`, `HasCustomAudience`, `HasRestrictedAudience`, `SyncAudienceCodec`, `SyncAudienceMsg` and `MsgId.SyncAudience`. A protocol-19 client is not told when it leaves an audience, but it is sent nothing more of the state. See the private sync state guide and `docs/sync-audience.md`. `SyncAudienceTests` and conformance scenario 27 cover it, in both tiers. (NEB-321)

- **Saved state for procedurally placed objects.** A game that generates resource nodes, plants or props from a chunk's seed can now record what players changed about them without making each one an entity. `ChunkState.TryGet(chunk, objectId, out ObjectState state)` reads an object's entry on any role, and `ChunkState.Changed` reports every change, expiry and arrival so clients can draw a mined rock as mined. A worker writes through `NebulaWorker.ChunkStates`: `Set`, `Clear` and `CompareAndSet` are applied by the worker that holds the chunk's lease, which a worker without the lease reaches through a worker message, so two players cannot both take the last unit of a node. An entry holds a value, an optional payload of up to 64 bytes and an optional expiry time in UTC Unix milliseconds, after which the object is untouched again; the time is compared when the chunk is next read or loaded, so nothing runs while it is unloaded. The entries of one chunk ride one small server-driven entity, spawned from a built-in prefab the first time an object in the chunk changes. It is saved and restored with the chunk, survives the chunk's release and its scope's retire and restore, and replicates to clients near the chunk. An untouched chunk writes no record and spawns nothing, and a chunk whose last entry is cleared or expires loses its entity and its record. Nebula now reserves network prefab ids from `NetworkPrefabs.BuiltInBase` (0xFF00) for its own prefabs. See the procedural object state guide and `docs/chunk-state.md`. (NEB-276)
...
- **Play without a build: the Multiplayer Play Mode run mode.** Set `NebulaConfig.EditorRunMode` to `MultiplayerPlayMode` and enable one virtual player in Unity's Multiplayer Play Mode. When you press Play, the virtual player hosts the whole server in its own Editor process: orchestrator, gateway and one worker, with the control plane in memory and autoscaling off. The main Editor is a client that connects to it. A virtual player tagged `Client` is one more client, and one tagged `Server` hosts. The server renders nothing: it turns off every camera, since it has nothing to show and a virtual player may not be able to compile the render pipeline's shaders. `Mesh`, the default, keeps today's behavior. An explicit `-nebula-role` and any other switch still win, builds ignore the setting, and an Editor without Multiplayer Play Mode falls back to `Mesh` with a warning. The boot log says which side a process took: `editor run mode: Multiplayer Play Mode (this virtual player hosts the server)`. New: `NebulaEditorRunMode`, `EditorRunPlan`, `EditorPlayer`, and `NebulaBootstrap.RunPlan`. On Unity 6.6 and later, Nebula reads the player from the engine and needs no package reference. `EditorRunModeTests` covers the decision. (NEB-243)
- **The Multiplayer Play Mode server keeps its saves in the project.** `NebulaConfig.EditorPersistence` chooses what the server a virtual player hosts does with persistent entities. `DevSaveFile`, the default, keeps them in `Library/Nebula/DevSaves/world.bin` from one Play to the next. The file is apart from anything a mesh started with `nebula start` keeps. `InMemory` starts every Play from a fresh world. **Nebula > Dev Loop > Reset Dev Saves** deletes the file, and refuses while the Editor is playing. A virtual player runs from a copy of the project in `Library/VP/<player>`, so `EditorDevPaths.ProjectRoot` walks its data path back to the main project. `-nebula-persistence-mode` and `-nebula-persistence-file` still win. (NEB-318)
- **The Multiplayer Play Mode server signs identities with a key of the project's.** A virtual player's persistent data folder is not the main Editor's, so its gateway made a new player signing key, and the main Editor's saved identity was refused once every session ("anonymous token signature does not match this mesh"), which cost a reconnect. The server a virtual player hosts now keeps the key in `Library/Nebula/dev-auth.key`. Builds and deployed meshes never read it. `NebulaGateway.AuthKeyPath` chooses the key file of any gateway started in Unity, and applies only when neither `AuthSigningKey` nor `MeshToken` is set. (NEB-319)
- **A Multiplayer Play Mode client joins only the server its own session started.** Before, when the virtual player's server could not bind its port because a mesh started with `nebula start` held it, the main Editor joined that other mesh without a word. Now both sides of the dev loop add `NebulaConfig.EditorPortOffset` (50 by default) to the gateway, worker and dashboard ports, so the dev loop and a mesh started with `nebula start` can run side by side. With the default ports, the dev loop uses udp/7050, udp/7151 and dashboard port 7130. The server writes `Library/Nebula/dev-session.json` once its ports are bound, or with the reason they are not, and removes the file when it stops. A client waits for that file and connects only to the port it names, and only while the server that wrote it is running. When the server failed, the client logs its reason and how to fix it, and connects to nothing. A client with no server after 20 seconds says how to start one: enable a virtual player in **Window > Multiplayer > Multiplayer Play Mode**, or set `EditorRunMode` to `Mesh`. New: `EditorDevSession`. `EditorDevSessionTests` covers it. (NEB-320)
- Documentation: "Iterate without building: Multiplayer Play Mode" in *Running locally* covers what you need, the setting, saved entities, ports, what one worker cannot test, and why the server renders nothing. The configuration guide lists the three new fields, and the pages that say a process is a client or a worker, never both, now point to the dev loop, where that rule still holds for each process. (NEB-248)

### Fixed

- **A pawn given its record with `Apply` had every save refused.** The documented way to bring back a player is to spawn the pawn, `Load` its record and `Apply` it. `Apply` left the pawn at the epoch it spawned at, 1, while its record carried the epoch of the player's previous pawn, which is higher after any handover. The store keeps a save only when its epoch is at least the stored one, so it refused every checkpoint of the pawn for the whole session, and logged that only with verbose logging. The Editor dev loop hit this on every Play once a player's record had passed epoch 1. `Apply` now continues the record's epoch as a restore does: a pawn this worker owns moves to `record.Epoch + 1` if it is lower, an entity applied before it is spawned spawns at that epoch whichever spawn call is used, and either is saved on the next checkpoint pass. A save from an older copy of the entity is still refused. The store now logs the first refused save of each key as a warning with both epochs, and `LocalPersistenceStore.StaleSavesDropped` and `SqlPersistenceStore.StaleSavesDropped` count them. Apply the record right after the spawn: an `AuthorityRpc` sent to the pawn at its old epoch can be rejected as stale, and a transfer prepared before the change no longer commits. `ConformanceAppliedRecordEpochTests` covers it; `docs/persistence-durability.md` D13 is the design record. (NEB-325)
- **The Multiplayer Play Mode server's log grew by gigabytes.** On Unity 6000.6, a virtual player cannot build the Universal Render Pipeline, and its Game view tried again on every draw, so the server's virtual player logged "Cannot create required material because shader is null" and a `MissingReferenceException` from `Blitter.Initialize`, with stack traces, every frame. One evening's log reached 10.5 GB. Turning the cameras off did not help: the Game view builds the pipeline before it looks at any camera. When a virtual player hosts the server, Nebula now closes its Game view as Play starts and puts a **Nebula Server** tab in its place. The Game view comes back when that player next plays as a client. The main Editor, a main Editor tagged `Server`, and clients keep their Game views. Nebula does not change the render pipeline setting instead: a virtual player shares the main Editor's `ProjectSettings` folder, and a pipeline set from a script is saved into it when the Editor saves or quits. New: `EditorRunPlan.HidesGameView` and `Nebula.Editor.ServerPlayerView`. `EditorRunModeTests` covers the decision. (NEB-326)
...
- Wire, additive: `AuthorityTransfer` gains a trailing optional session section after the `crossing` byte. The sender writes it only when the owner's session is waiting to be reclaimed. The protocol stays 19, and every worker of a mesh must still run the same build. (NEB-309)

### Fixed

- **`NetworkIdentity.PlaceInScope` put an entity tens of metres off when the target was inside a physics frame.** On a worker, an entity inside a frame (a ship's interior) has frame-local coordinates on its transform. `PlaceInScope` moved the entity into the container at the target, then wrote the scope pose straight onto the transform, so a warp to a point inside a ship landed at those numbers in the ship's frame. It now converts the pose into the new container's frame after the container change. Instance transfers had the same mistake: `NebulaWorker.TryCommitTransfer` converts its pose into a destination inside a frame, and `TryCommitTransfers` reads each member's pose in the scope's own space, so a rider that leaves its framed ship behind arrives where it stood. `ConformancePhysicsFrameTests` and `ConformanceCrossScopeCrewTests` cover it. (NEB-310)
- **A disconnected player's pawn was never removed if it changed worker during the reclaim window.** A worker keeps a disconnected player's pawn for `SessionReclaimSeconds` and then removes it. The handover did not say that the player was gone, so the next worker treated the player as connected and kept the pawn, and the chunk it stood in, forever. `AuthorityTransferMsg` now carries the session's state (`SessionOrphan`) and the seconds of the grace it had left (`SessionReclaimRemaining`). The next worker resumes the countdown on its own clock and removes the pawn when the grace runs out, unless the player reconnects first. Two related gaps are closed too. A worker that hears the player left before the handover arrives keeps that news. A gateway whose link to a worker drops and comes back no longer revives the sessions of players who had left through it. New: `PlayerSessions.Session.Released`, `PlayerSessions.OrphanKind`, and `PlayerSessions.ReclaimRemaining`. Conformance scenario 26 (`ConformanceSessionHandoverTests`) covers it. (NEB-309)
- **Chunk leases did not follow a player aboard a ship with its own physics frame.** On a worker, the root of a physics frame sits at Unity's origin, and `RuntimeGrid.CoordOf(NetworkIdentity)` read a rider's cell from its frame-local transform. So the allocator kept the pilot's ring of leased chunks around cell (0,0) or (-1,-1) wherever the ship flew, and the server-driven ship pulled no ring of its own. A rider's cell is now worked out from its position in the scope's own space (`NetworkIdentity.ToScope`), through every frame between it and the chunk, so the ring follows the outermost carrier. Riders of carriers without a frame, entities directly in a chunk, and entities in no container get the same cell as before. `CoordOf` also reads a cell from a container only when the container is a root chunk of the grid's own scope. Before, the public grid unpacked the id of any runtime container, such as a room fixed inside a ship, into a cell it was not. (NEB-306)
- **A client aboard a carrier followed the wrong floating origin.** A client keeps the origin of the grid its pawn stands in (`docs/scope-frames.md` D6). The pawn of a pilot is in the ship's container, which is no chunk, so the client fell back to the public grid, and in a scoped world it re-centred the public origin instead of the world's own. It now follows the grid of the chunk at the bottom of the pawn's carrier chain. `RuntimeGridRiderTests` covers both fixes. (NEB-306)
...
- **The dashboard shows the container tree.** The **Containers** table lists each container under its parent and marks inherited containers (with the ancestor whose worker simulates them), leased containers demoted under a moving parent, runtime containers, and physics frames. The **Carried containers** table shows frames and hides the pin controls of a container authored as leased. The World map draws containers fixed inside a carrier where the carrier is, and its inspector says how each container is simulated. Rows in `/api/state` gain `parent`, `depth`, `source`, `authority`, `mode`, `leased`, `ownerFrom`, `frame`, and `interest`.
- `POST /api/containers/ensure` accepts `parent`, `authority`, `frame`, and `interest`, and `NebulaOrchestrator.EnsureRuntimeContainer` has an overload that takes a `ContainerPlacement`.

### Changed

- **Breaking (protocol 19, not additive):** `DynamicContainer` is folded into `Container`. A `Container` on the root of an entity, next to its `NetworkIdentity`, is carried by that entity: `NetworkIdentity` registers the box when the entity spawns and unregisters it, setting its riders down, when it despawns. `Container.FrameMode` is read from the object and is never authored. `Container.OfRigidbody` and `Container.IsCarrierGeometry` replace the `DynamicContainer` statics, and `Container.Entities` replaces `DynamicContainer.Contents`; `NetworkIdentity.Carried` is unchanged. Removing the behaviour renumbers the network behaviours of every carrier prefab, which address RPCs, variables and sync state, so `HelloMsg.ProtocolVersion` and `HelloMsg.MinProtocolVersion` are both 19: gateways refuse protocol-18 clients with `ProtocolUnsupported` and their range. Rebuild clients and servers together.
- `DynamicContainer` is an `[Obsolete]` `MonoBehaviour` that does nothing, kept for one release so projects still load. **Nebula > Migrate > Remove DynamicContainer** strips it from every prefab and open scene; remove `[RequireComponent(typeof(DynamicContainer))]` from your scripts first.
- Placement is validated instead of required: the `Container` inspector, `OnValidate` and **Nebula > Validate Project** warn about a `Container` on a child object of an entity (it is baked as a fixed box and does not move with the entity) and report an error for one on an entity's root without a `NetworkTransform`.
- `ContainerRegistry.Rebuild`, the service export and the world baker skip containers on an entity's root. The world baker used to bake a carrier placed in a cell scene as a fixed container.
...
- Wire, additive (these fields alone would have kept protocol 18; the release is protocol 19 for the breaking change above): lease rows gain `parent`, `authority`, `frame`, and `interest` and write `center` in double precision; `ContainerOwnership` gains a trailing placement section; `AuthorityTransfer` gains a trailing `crossing` byte.

### Fixed

- **Doors, seats, and ramps of a framed carrier were missing from its physics frame.** A part whose `NetworkBehaviour` pulled in a `NetworkIdentity` of its own (through `RequireComponent`) was skipped as another entity, so a pawn inside the frame walked through closed doors, fell through a lowered ramp, and could not aim at a seat or a button. Only colliders of entities riding under the carrier are skipped now: an identity nested in the carrier's prefab is never spawned, and its colliders are the carrier's.
- **A client predicted against stale copies of a frame's colliders.** Only workers kept the copies in step with their sources, so a door that opened on a client stayed closed in the frame its pawn predicted in. Clients now sync them every rendered frame, and after posing the frames for rendering they update the physics scene, so a crosshair ray meets a frame where it is drawn.
- **A predicted entity lost its simulation state in a handover.** The next worker took the pose from the ghost stream and nothing else that `Simulate` carries from tick to tick, so a pawn in the middle of a jump or a jetpack climb dropped at a container seam. The state `WriteState` describes now travels with the pending inputs, and `ReadState` applies it on the receiving worker. A worker that does not send it is still understood.
- **The World map drew what is inside a physics frame at the frame's coordinates.** Entities aboard a framed carrier, and containers inside a frame, were reported at their frame-local position; they are now converted to the scope first.
- **The standalone orchestrator dealt containers that are simulated as inherited.** A runtime container fixed in a carrier without a physics frame is simulated by the carrier's worker (docs/container-tree.md D7), but the service orchestrator counted it as leased and assigned it to a worker of its own. It reads the carrier's frame from its lease row now.
- **A worker declared dead while it was still running could leave two live copies of its persistent entities.** A worker that only missed heartbeats, for example because of a network partition or a stall, was declared dead. Its containers were dealt to another worker, which restored their entities from the last checkpoint while the old copies were still simulating. For a moment each entity had two authoritative copies, and the restored one was up to a checkpoint interval stale. The old worker's late saves could also overwrite the restored state. Nebula now keeps at most one copy (design: `docs/persistence-durability.md` D7–D12):
  - A worker **fences** itself when its own control-plane heartbeat is older than `WorkerTimeoutSeconds`. The orchestrator uses the same test to declare a worker dead. A fenced worker keeps simulating, but it does not save, restore, or hand entities over. It does not retire a scope part, and `ReleaseRuntimeContainer` returns false. Dirty entities are saved when the next heartbeat lands. New `NebulaWorker.IsFenced` and `WorkerRegistration.IsFenced`.
  - A restored entity is saved on the next checkpoint pass at its new epoch, instead of waiting for its checkpoint interval. After that save, the store refuses any save the old worker still had queued.
  - A worker that learns it was declared dead despawns everything in the containers now dealt to other workers. It neither saves nor deletes those entities, and then it registers again. A control plane that came back empty is not treated as a death. New `WorkerRegistration.IsDeclaredDead`.
  - Gateways and workers drop their link to a worker the control plane declared dead, even while the link is up. The gateway forgets that worker's entities, so a client is never shown the old copy beside the restored one. New `WorkerRoster` reports these workers.
  - A restarted orchestrator that is away for longer than `WorkerTimeoutSeconds` now delays saves and handovers until the workers' heartbeats land again. Nothing is lost.
  - Conformance scenario 19, `ConformanceDeclaredDeadWorkerTests` (Unity tier) and `DeclaredDeadWorkerTests` (.NET tier, with the gateway), covers the fix. (NEB-256)
- **Riders a worker owned aboard another worker's carrier outlived a retired chunk.** `NebulaWorker.EmptyContainer` (a chunk retired, a scope part checkpointed) despawns a vehicle's riders with the vehicle and saves them aboard it, but only the riders its own worker owns. A rider can be owned by a worker that holds the carrier as a ghost, for example when the carrier's interior is pinned to that worker. That worker heard an ordinary despawn and put its riders down in the box around the carrier, a box that was itself being unloaded. The ghost despawn now tells the ghost's worker why the carrier left. When the carrier's owner emptied its box, the ghost's worker empties the ghost's box the same way: riders before their carriers, each persistent one saved aboard under the carrier's own record so it comes back with the carrier, transient ones lost. When the carrier was despawned any other way, the riders are still put down where it stood, as the owner does with its own (docs/dynamic-worlds.md D5) (NEB-258).
  - Wire, additive: `EntityDespawnMsg` has two trailing optional fields, `TakesRiders` and `CarrierKey` (the carrier's `PersistentEntity` key, which a ghost does not otherwise know). They are written only on a worker-to-worker despawn that takes riders; every other despawn keeps its bytes.
  - Conformance scenario 18 has a two-worker leg, `ConformanceGhostCarrierRidersTests`.
- **A carrier leaving a scoped chunk could put its passengers down in the public world.** When a dynamic container is unregistered, `ContainerRegistry` moves whatever is still inside it into the container around it. It looked for that container among public containers only, so a passenger of a ship in an instance or a scoped grid could land in a public container that happened to hold the same point, crossing a scope boundary that only `InstanceBoundary` may cross. It now looks only among the boxes of the carrier's own scope: the smallest one holding the passenger, or that scope's nearest box when none does. The worker's and the client's own evacuation paths already put riders into the carrier's own container, which is in the same scope (NEB-255).
- **Loose cargo in a crewed ship flickered for its riders when the ship changed scope.** A rider's gateway follows the carriers its pawn rides in by name (`docs/scope-activation.md` D12), so a ship crossing into another scope stays published to it. What else was aboard, such as a crate in the cargo bay or another gateway's player, was not named. When the ship crossed, its worker rebucketed the contents under the new scope's region keys and told the rider's gateway to forget them. The rider's client despawned them, and they spawned again once its subscriptions followed the ship. The gateway now also names everything aboard the outermost carrier of each rider's chain, at any depth (D21). The contents stay published and cached with the carrier, and a rider's client keeps them through the crossing. Clients left in the old scope still lose the ship and its cargo before anything about the destination is sent (D13). Entities owned by the gateway's own clients are not named, because they are already kept as their owner's. New `NebulaGateway.MaxFollowedPassengers` (4096) bounds how many passengers one gateway names per subscription pass. Cargo past the bound is served by region as before. No wire change; protocol stays 18. `ConformanceCrossScopeCarrierTests.CargoAboardACrossingShipStaysWithItsRiderAndLeavesTheOnlooker` covers the fix (NEB-261).
- **A carrier crossing into a chunk the gateway had not heard of yet could arrive with stale variables.** Since the crewed-carrier crossing fix, the gateway holds a spawn or state entry that names a runtime container whose lease row has not reached it yet (`docs/scope-activation.md` D15). Variables, behavior state and RPCs that the entity's new owner sent during that hold were not held with the spawn. The ordinary owner check dropped them, because the gateway's record still named the previous owner. If a redirect had already named the new owner, they were relayed ahead of the spawn instead, and the spawn's older variables then overwrote them on every client. Either way a ship could arrive with an old speed cap or gear state until the next change. Now every variable, behavior-state, RPC and despawn message for an entity with held updates waits behind them and is applied in arrival order right after the spawn lands. A message of an older epoch than everything held (the previous owner's last messages) is handled as before. Released behavior state goes out on the reliable stream behind the spawn. A state entry for an entity the gateway knows only as a held spawn now waits too, instead of being dropped as unknown. These messages count toward the same bound as the rest of the hold: 10 s and 1024 updates per entity. No wire change; protocol stays 18. `ConformanceCrossScopeCarrierTests.ANewOwnersChangesDuringAHeldSpawnArriveAfterIt` covers the fix (NEB-257).
- **Every carrier that spawned read its cargo with a request of its own.** When a worker spawned a carrier with authority, `NebulaPersistence` asked the store for what was saved aboard it straight away, one `LoadCarried` read per carrier. Since the restore-burst fix those reads waited behind two reader threads, so a burst could no longer starve the worker, but twenty carriers spawning together, or a container restore that brought back a hangar full of vehicles, still cost twenty round trips to the orchestrator before anything aboard came back. Carrier reads are now batched like container restores (NEB-254).
  - `NebulaPersistence` marks each carrier that spawns with authority as due, and each frame reads the due carriers together: at most `MaxCarriersPerRestoreLoad` (256) per read and at most `MaxRestoreLoadsInFlight` (2) carrier reads waiting on the store. The cargo comes back one frame after its carrier spawns. An answer is used only for a carrier that is still the one that asked: a carrier that despawns, or spawns again, while its read is out gets nothing from the stale answer. A carrier spawned while the store is unreachable is read once it connects, where before its cargo was never read. New `NebulaPersistence.CarriedLoadsInFlight`.
  - **Breaking:** `IPersistenceStore.LoadCarried(carrierKey, onLoaded)` is replaced by `LoadCarried(carrierKeys, onLoaded)`, which answers once with an entry for every distinct key asked for (an empty list when nothing is saved aboard, and always for the empty key). Pass one key to read one carrier. `LocalPersistenceStore` answers from one pass over its records, and `SqlPersistenceStore` from one job with a chunked `WHERE carrier_key IN (...)` query.
  - **Breaking (wire):** the orchestrator's `GET /api/store/carried?key=` is replaced by `POST /api/store/carried` `{"keys":[...]}`, which answers with one `{"records":[...]}` list for up to `PersistenceHost.MaxCarriersPerLoad` (256) carriers and refuses a longer list with HTTP 400. `RemotePersistenceStore.LoadCarried` splits a longer list into requests of 256 keys and still answers once. Workers and the orchestrator must run the same build.
  - Conformance scenario 17, `ConformanceRestoreBurstTests`, gains the carrier cases in both tiers.
- **A game that filled the thread pool could get its worker declared dead.** `RemoteControlPlane` runs its long poll and its writes (heartbeats among them) on dedicated threads, but each one waited on `HttpClient.SendAsync`, whose completion needs a thread-pool thread. When the game queued enough blocking work to fill the pool, the mirror froze and heartbeats stopped until the pool drained. The orchestrator declared the worker dead after `WorkerTimeoutSeconds`, and nothing in the worker's log said why. The mirror's own threads no longer depend on the pool (NEB-253).
  - The reader and sender threads now do their HTTP with blocking socket calls on their own threads, through a small internal HTTP/1.1 client with one kept-alive connection each. They no longer need the thread pool. The mesh token header, the 20-second timeout, `https` addresses, the retry and drop rules for writes, and cutting a request short on `Dispose` are unchanged. These connections no longer count against the per-address connection limit that `NebulaHttp` raises for the other clients.
  - `Tick` logs a warning when a write has waited longer than the new `RemoteControlPlane.StallWarningSeconds` (default 5, set from `WorkerTimeoutSeconds` by the bootstrap and the standalone gateway), or when no document has arrived for `DisconnectAfterSeconds`. The warning includes the thread pool's state: busy and maximum threads and, on .NET, the number of waiting work items. A second message reports the recovery.
  - `RemotePersistenceStore`, worker telemetry and gateway session requests still use `HttpClient`. A stall there delays a restore, a dashboard update or an admission, but it does not get the worker declared dead.
- **An entity could outlive the release of the chunk it was in.** The chunk allocator released an unwanted chunk while an entity filed under it was still simulated by another worker (a handover in flight, or a peer not yet connected): the owner saw only a ghost, emptied the chunk without it, and the other worker then moved the live entity into the nearest box instead of it being checkpointed and despawned with the chunk. The next lease of that chunk found the entity's old record. `RuntimeGridAllocator` now keeps a chunk while a ghost is filed directly in it, and a worker that finds one of its own persistent entities in a runtime box another worker released logs a warning naming both (NEB-262, `docs/dynamic-worlds.md` D4).
- **A custom scope retire policy saw no players when every player was aboard a vehicle.** A worker reports an entity riding in a vehicle under the vehicle's own carried container, which is right for cost, but `ScopeLifecycle.Occupancy` summed only the scope's own containers, so `ScopeRetireContext.Players` read 0 for a scope whose players were all seated in ships. The default policy was not affected, because the vehicle itself counts as an entity. A carried container's telemetry row now names the container its vehicle is in (new `ContainerLoad.Enclosing`, an additive `enclosing` key in the worker's document), and `Occupancy` over a scope's parts adds each carried container's counts to the part its vehicle is in, through vehicles inside vehicles. Cost and assignment readings are unchanged (NEB-259).
- **A chunked-world scope could retire with players still in it.** The scope lifecycle judged a scope on the containers its row names. A `ScopeKind.Grid` scope's row names only its anchor chunk; every other chunk is leased on demand around the players. So a player a few chunks from the anchor was invisible to the idle sweep: after `ScopeIdleRetireSeconds` the default policy retired the scope, admission stopped, and only the anchor was checkpointed and released while players stood in the other chunks. The lifecycle now works over a scope's **live parts**, the row's containers plus every lease row carrying the scope's key (NEB-252, `docs/scope-lifecycle.md` D4).
  - Idle age, occupancy and the retire sequence cover every live part. Each chunk gets the before-retire window, the forced checkpoint, emptying and an acknowledgement; the orchestrator waits for all of them and releases every chunk's lease on `Retired`. A restore still waits for the row's parts only: re-activation recreates the anchor, and the other chunks restore their saved entities when they are leased again.
  - New `ScopeLifecycle.CollectParts` and `IndexParts`, and overloads of `IdleSeconds`, `Occupancy` and `NextState` that take the live parts. The overloads that take only the row keep their meaning. `AckScopePart` accepts an ack for any live part. The dashboard's scope card shows leased chunks next to the row's parts (`liveParts` in `/api/state`).
  - A worker that requests one of a scope's chunks re-stamps its lease row even when it owns the chunk, so the row's age says when the chunk was last wanted. Public-world chunks are unchanged.
  - A pinned chunk is only kept in existence and never re-stamped (`RuntimeGridAllocator.AddPin`). Every worker that knows a grid scope pins its anchor, and a non-owner's re-stamps kept any grid scope known to two workers from ever going idle.
  - The chunk allocator no longer releases any chunk of a retiring scope; they all belong to the retire sequence.
  - A scoped runtime container no longer refuses to unregister because of entities whose objects were destroyed without a despawn.
- **The SQL persistence store could leave a caller waiting forever.** `SqlPersistenceStore` retried a failing job three times and then dropped it without invoking its callback. A worker talking over HTTP recovered by retrying its request, but an in-process caller (the orchestrator's own tools, the restore drill, tests) waited for an answer that never came. The store now answers every read and every `WhenWritten` barrier exactly once (NEB-260).
  - A read the store gives up on is answered with the empty result (null record, empty list, an empty entry per container, a count of 0), and a barrier is released. New `PersistenceAnswer.Failed` is true while such a stand-in callback runs, so a caller can tell "unreadable" from "nothing saved". A barrier is also marked failed when the store gave up on a save or delete queued since the previous barrier.
  - `PersistenceHost` answers a stand-in with HTTP 503 instead of an empty 200, so a worker retries instead of restoring nothing.
  - A job whose own work succeeded is no longer run again (and answered twice) when the record-count refresh after it fails.
- **A Windows gateway could not serve HTTPS with a certificate from a private ACME CA.** Windows' chain engine can refuse an offline chain build outright ("An unknown chain building error occurred") when the chain's root is not in the machine's store. `WebCertificates` built the gateway's certificate context offline, so it discarded every certificate such a CA issued and retried forever. It now falls back to an online build on Windows, which accepts the same chain. Let's Encrypt, whose root Windows trusts, was not affected. `AcmeTests.TheGatewayGetsAnIpAddressCertificateOverAcmeAndServesHttpsWithIt` passes on Windows again; its client now trusts the test CA inside the handshake's own chain build (NEB-263).
...
- **A ship leaving a client's view destroyed the replicas riding in it.** Riders are parented under their carrier, and `NebulaClient` destroyed the carrier's object without putting them down first, as the worker does. The riders' replicas were destroyed but stayed in the client's entity table, so every later frame's `RemoteTick` and the riders' own despawns threw `NullReferenceException`. The client now moves riders into the carrier's own container before destroying it, and drops (with a warning) any replica whose object was destroyed without a despawn.
- **A worker dealt hundreds of containers at once could be declared dead, and take the next worker with it.** When a worker gained a batch of leases (a re-deal after another worker died, a rebalance along a boundary, a burst of runtime chunks), it sent one persistence load per container to the orchestrator, all at the same time, each on the thread pool. In a Unity (Mono) player a few hundred of them starved the thread pool: no load completed, and neither did the control-plane long poll or the heartbeat, which share that pool and the same orchestrator address. After `WorkerTimeoutSeconds` the orchestrator declared the worker dead and dealt its containers to the next worker, which received the same burst. A sample with fast ships and a runtime chunk grid lost every worker this way. Restores and every other persistence read are now bounded.
  - `NebulaPersistence` reads the containers that are due in batches: at most `MaxContainersPerRestoreLoad` (256) per read and at most `MaxRestoreLoadsInFlight` (2) reads waiting on the store. Containers beyond that wait for a later frame. The restore rules are unchanged: the grace period, the scope's activation gate, the lease-moved-on check, and an answer used only when its request is still the one on record. New `NebulaPersistence.RestoreLoadsInFlight`.
  - **Breaking:** `IPersistenceStore.LoadContainer(containerId, onLoaded)` is replaced by `LoadContainers(containerIds, onLoaded)`, which answers once with an entry for every distinct ID asked for (an empty list when nothing is saved there). Pass one ID to read one container. `LocalPersistenceStore` answers from one pass over its records, and `SqlPersistenceStore` from one job with a chunked `WHERE container_id IN (...)` query.
  - **Breaking (wire):** the orchestrator's `GET /api/store/container?id=` is replaced by `POST /api/store/containers` `{"ids":[...]}`, which answers with one `{"records":[...]}` list for up to `PersistenceHost.MaxContainersPerLoad` (256) containers and refuses a longer list with HTTP 400. Workers and the orchestrator must run the same build.
  - `RemotePersistenceStore` sends every read (`Load`, `LoadContainers`, `LoadCarried`, `LoadWhere`, `CountRecords`) from a queue served by `MaxConcurrentReads` (2) reader threads, so it never has more than two reads in flight, whatever it is asked for. It splits a longer `LoadContainers` list into requests of 256 IDs and still answers once. New `PendingReads`. A retry now waits the full `RetrySeconds` instead of being cut short by every new save.
  - Every request from a worker or gateway to the orchestrator (control plane, persistence, telemetry, gateway sessions) raises that address's connection limit to 32 first. Under Mono the limit defaults to two per address for the whole process, and the control plane's long poll holds one of them.
  - Conformance scenario 17, `ConformanceRestoreBurstTests` (Unity tier for the worker, .NET tier for the stores, the endpoint and the in-flight bound), covers the fix.
- **The chunk allocator despawned crewed ships from under their pilots.** `RuntimeGridAllocator` decided whether a chunk was occupied from the entities directly in it. A pilot seated in a ship's dynamic container is one level down, so a chunk holding a crewed ship read as empty. A sample with fast ships showed this: chunk leasing fell behind the ships, each ship stayed assigned to the last chunk it had left (container resolution keeps an entity in the nearest box when no box holds it), and that chunk dropped out of the pilots' ring. After `RetireAfterSeconds` the allocator released the chunk and despawned every ship, and their pilots were left frozen in seats that no longer existed.
  - A chunk is occupied while a client owns an entity anywhere inside it, at any vehicle depth: a pilot in a ship, or a passenger in a shuttle in that ship's hangar.
  - A chunk is also kept while an authoritative entity assigned to it stands in a cell the allocator wants, or in a cell whose lease row another worker touched within `RetireAfterSeconds`. The entity moves into that cell once it is registered, and the old chunk then retires normally. Content in space nobody wants is still unloaded with its chunk. Design: `docs/dynamic-worlds.md`, "Retiring a chunk under a carrier".
  - `NebulaWorker.EmptyContainer` (and so `ReleaseRuntimeContainer` and a scope part's retire) takes a vehicle's riders with the vehicle, at every depth. Riders are despawned before their carrier, and each persistent rider is checkpointed aboard the carrier, so it comes back with it. Before, only the box's own entities were despawned, and despawning a vehicle set its riders down in the box being emptied, where they outlived it.
  - A despawned entity now leaves its container's `Entities` list when it is despawned, not when its object is destroyed. A scoped runtime box that still listed a departed entity could not be unregistered. The worker destroys a despawned entity's object with `DestroyImmediate` outside play mode.
  - `WorkerScopeLifecycle.IsBusy` and `NebulaPersistence.CheckpointContainer` count riders: a client's pawn or unsaved state aboard a vehicle keeps a scope part busy, and the forced checkpoint of a retiring part saves the riders too.
  - Conformance scenario 18, `ConformanceCrewedCarrierRetireTests` (Unity tier), covers the fix.
