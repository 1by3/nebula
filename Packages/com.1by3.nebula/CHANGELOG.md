...
- The worker telemetry document gains `holds` (container id and seconds remaining) and `cohesion` (group, members, containers) arrays. The orchestrator turns a reported hold into a deadline on its own clock, so the two processes need no common time base; a hold dies with the worker that asked for it.
