...
// Frozen protocol-19 oracle: do not replace literals or readers with production protocol types.
// Protocol 19 changed no frame layout: it renumbered the behaviours of carrier prefabs (the obsolete
// DynamicContainer behaviour left them), so this is the protocol-18 oracle with its version literals raised.
...
// are not compatibility claims made by this fixture. Like the protocol-19 client, unknown
...
internal sealed class Protocol19GatewayDecoder
{
    ...
    {
        if (depth > 8) throw new InvalidDataException("protocol-19 batch nesting limit");
        ...
        switch (id)
        {
            case 2: // Welcome
                ...
                r.Bool();
                NegotiatedVersion = r.Remaining == 0 ? (ushort)19 : r.U16();
                if (NegotiatedVersion != 19) throw new InvalidDataException("gateway did not negotiate protocol 19");
                Welcomes++;
                break;
            case 5: // JoinStatus
                byte state = r.Byte();
            ...
            case 16: // ContainerOwnership
                ...
                for (int i = 0; i < upserts; i++)
                {
                    ...
                    if (r.Bool()) throw new InvalidDataException("instance containers are outside the protocol-19 replay coverage");
        ...
        // Dispatch in the protocol-19 client discards the remainder of each bounded message.
    ...
    private sealed class Cursor : IDisposable
    {
        ...
        {
            byte value = Byte();
            if (value > 1) throw new InvalidDataException("invalid protocol-19 boolean/flags");
            return value != 0;
        }
        internal byte[] Bytes(int length)
        {
            if (length > Remaining) throw new EndOfStreamException("truncated protocol-19 field");
        ...
        private float Float()
        {
            ...
            if (!float.IsFinite(value)) throw new InvalidDataException("non-finite protocol-19 transform");
            return value;
        }
        internal float[] Axes(ushort fields, int shift)
        {
            ...
            for (int i = 0; i < 3; i++)
                if ((fields & (1 << (shift + i))) != 0)
                {
                    ...
                    if (!float.IsFinite(values[i])) throw new InvalidDataException("non-finite protocol-19 transform");
                }
            return values;
        }
