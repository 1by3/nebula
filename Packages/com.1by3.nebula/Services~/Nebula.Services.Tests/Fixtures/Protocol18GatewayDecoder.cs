...
// Frozen protocol-18 oracle: do not replace literals or readers with production protocol types.
...
// are not compatibility claims made by this fixture. Like the protocol-18 client, unknown
...
internal sealed class Protocol18GatewayDecoder
{
    ...
    {
        if (depth > 8) throw new InvalidDataException("protocol-18 batch nesting limit");
        ...
        switch (id)
        {
            case 2: // Welcome
                ...
                r.Bool();
                NegotiatedVersion = r.Remaining == 0 ? (ushort)18 : r.U16();
                if (NegotiatedVersion != 18) throw new InvalidDataException("gateway did not negotiate protocol 18");
                Welcomes++;
                break;
            case 5: // JoinStatus
                byte state = r.Byte();
            ...
            case 16: // ContainerOwnership
                ...
                for (int i = 0; i < upserts; i++)
                {
                    ...
                    if (r.Bool()) throw new InvalidDataException("instance containers are outside the protocol-18 replay coverage");
        ...
        // Dispatch in the protocol-18 client discards the remainder of each bounded message.
    ...
    private sealed class Cursor : IDisposable
    {
        ...
        {
            byte value = Byte();
            if (value > 1) throw new InvalidDataException("invalid protocol-18 boolean/flags");
            return value != 0;
        }
        internal byte[] Bytes(int length)
        {
            if (length > Remaining) throw new EndOfStreamException("truncated protocol-18 field");
        ...
        private float Float()
        {
            ...
            if (!float.IsFinite(value)) throw new InvalidDataException("non-finite protocol-18 transform");
            return value;
        }
        internal float[] Axes(ushort fields, int shift)
        {
            ...
            for (int i = 0; i < 3; i++)
                if ((fields & (1 << (shift + i))) != 0)
                {
                    ...
                    if (!float.IsFinite(values[i])) throw new InvalidDataException("non-finite protocol-18 transform");
                }
            return values;
        }
