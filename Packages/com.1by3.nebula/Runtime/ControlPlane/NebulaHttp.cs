...
{
    /// <summary>
    /// HTTP requests from a worker or gateway to the orchestrator: the control-plane mirror
    /// (<see cref="RemoteControlPlane"/>), the persistence client (<see cref="RemotePersistenceStore"/>) and the worker's
    /// telemetry all reach the same dashboard address.
    /// <para>
    /// Under Unity's Mono runtime every request to one address shares one <c>ServicePoint</c>, whose connection limit
    /// defaults to two for the whole process. The control plane's long poll holds one of them all the time, so
    /// heartbeats, checkpoints and restores would queue behind one another on the other. Every request built here
    ...
    {
        /// <summary>
        /// Connections one process may hold open to one orchestrator address: the control plane's long poll and
        /// sender, the persistence sender and readers, telemetry, and a gateway's session requests, with room to spare.
        /// </summary>
