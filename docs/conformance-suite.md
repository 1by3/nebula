...
|---|---|---|---|---|---|
| 1 | The same location triple resolves the same container on gateway, worker, orchestrator and client; survives handover, restart and a store round trip. | NEB-220 | A (+D for restart) | `Tests/EditMode/ConformanceLocationTests.cs` | in progress (NEB-220) |
...
| 5 | A cross-worker call is applied at most once when the target hands over between send and apply; a stale-epoch call is rejected with a reason. | NEB-221 | B | `Tests/EditMode/ConformanceCallContractTests.cs` | in progress (NEB-221) |
...
| 9 | A diagnostic fires for a cross-container joint without a cohesion group. | NEB-234 | C | `Tests/EditMode/ConformancePhysicsDiagnosticTests.cs` | in progress (NEB-234) |
