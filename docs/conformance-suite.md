...
| 16 | A crewed carrier crossing into a chunk of another grid scope: a group prepared for the ship and its crew becomes ready and commits with the crew in their seats; every rider's client follows the ship into the destination with no reconnect, also when only the ship crossed; an onlooker in the old scope loses it and is never sent the destination's row; no client is sent an update naming a container before its row. | crossing (`docs/scope-activation.md` §11) | A (fake mesh, real gateway) + B (the group commit) | `Services~/Nebula.Services.Tests/ConformanceCrossScopeCarrierTests.cs` (6 tests: a preparation into another scope sends the rider its destination row ahead of the request and the client answers ready, nobody else gets the row and no destination entity is revealed; a ship crossing alone takes its rider's client into space with no despawn while the onlooker loses it and never hears of the destination; a rider reconnecting after the crossing finds the ship in space by name; updates naming a chunk whose lease reaches the gateway late are held until the row arrives, so nobody loses the ship and no client is sent an unresolvable name; a new owner's variable, behavior state and RPC sent while its spawn is held arrive after the spawn and the rider ends with the new value (NEB-257); a crate in the ship's bay and a parcel in that crate cross with it and are never despawned for the rider, while the onlooker still loses them and never hears of the destination (NEB-261)) — it needs `MeshFixtures`, which only the service tests compile. The worker leg is `Tests/EditMode/ConformanceCrossScopeCrewTests.cs` (2 tier-B tests: a ship and two riders committed as one group arrive with the riders still seated, a new epoch each and their scope following the ship; a rider committed without its ship is put down in the destination). | covered |
| 17 | A worker that gains hundreds of containers at once restores every one of them with a bounded number of store requests: at most `NebulaPersistence.MaxRestoreLoadsInFlight` reads of at most `MaxContainersPerRestoreLoad` containers each, every container read once and judged with its own records, a container whose lease moved on while its read was out not restored from the stale answer; carriers that spawn together have their cargo read the same way (at most `MaxRestoreLoadsInFlight` reads of at most `MaxCarriersPerRestoreLoad` carriers each, each carrier once, a carrier that despawned while its read was out not restored from the stale answer); and the worker's persistence client never has more than `RemotePersistenceStore.MaxConcurrentReads` requests in flight, whatever it is asked for. | restore-burst fix (a worker re-dealt a few hundred containers fired one load per container, starved its thread pool, missed its control-plane heartbeats and was declared dead, and the next worker got the same burst); NEB-254 (one carried read per carrier) | C for the worker (a real `NebulaPersistence` over `LocalPersistenceStore` behind a pass-through that holds the answers; the carrier cases on a one-worker `ConformanceMesh` so the carriers and their cargo are really spawned), A for the store and the wire | `Tests/EditMode/ConformanceRestoreBurstTests.cs` (4 tests, Unity only: 600 containers dealt in one control-plane change are read in three batches, never more than two waiting, each container once, and every scene record lands against its own container; a container dealt away mid-read is skipped while the rest of its batch restores, and is read on its own when it comes back; 600 carriers spawned in one go have their cargo read in three batches, never more than two waiting, each carrier once, and every saved crate comes back aboard its own carrier; a carrier despawned mid-read gets nothing from the stale answer while the rest of its batch does, and is read on its own when it spawns again). The store and wire leg is `Services~/Nebula.Services.Tests/ConformanceRestoreBurstTests.cs` (7 tests: `SqlPersistenceStore` answers 601 container ids once from chunked `IN` queries on SQLite, carried and unrequested records excluded; the same for 602 carrier keys (`carrier_key IN`), the empty key answered empty; `RemotePersistenceStore` sends containers to a real `PersistenceHost` as three `POST /api/store/containers` requests and carriers as three `POST /api/store/carried` requests, each answered once; a burst of 600 single reads plus 1000 container ids never has more than `MaxConcurrentReads` requests open at a slow orchestrator; the host refuses an oversized or malformed container or carrier request and no longer serves the one-container or one-carrier endpoints). Not covered: the thread-pool starvation itself, which only a Mono player shows; the tests pin the bound that prevents it. | covered |
| 18 | A chunk is never retired from under a client's pawn at any carrier depth (a pilot in a ship, a passenger in a shuttle in that ship's hangar), nor from under an authoritative entity filed under it that stands in a cell this worker wants or another worker has leased; the same chunk with an empty ship in unwanted space is retired. A retired chunk takes a vehicle's cargo with the vehicle: riders are despawned before their carriers, none is set down in the deleted box, each persistent one is saved aboard its carrier, and the forced checkpoint and the busy rule of a scope part count riders too. Riders another worker owns aboard the carrier (it holds the carrier as a ghost) leave with it the same way and are saved under the carrier's own key; when the carrier is despawned any other way, that worker puts them down where it stood. | crewed-carrier retire fix (a sample with fast ships outran its chunk leases; the allocator retired the chunk each ship was still filed under and despawned the ships from under their pilots); NEB-258 for riders owned by another worker | B (`ConformanceMesh`, a real `RuntimeGridAllocator` over a `LocalControlPlane` with a driven clock) + C for the records (a real `NebulaPersistence` over `LocalPersistenceStore`) | `Tests/EditMode/ConformanceCrewedCarrierRetireTests.cs` (5 tests, 10 cases, Unity only: a ship 17 km past its only chunk keeps the chunk with its pilot aboard and loses it empty; in a retiring scope a client's pawn one and two carriers deep occupies the chunk and an empty ship does not; an uncrewed ship filed under a stale chunk keeps it while it stands in a wanted cell or in a cell another worker leased just now, and not in space nobody wants; releasing a chunk despawns a ship, the shuttle in its hangar, a crate in the shuttle and a transient drone, leaves nothing in the box and saves the shuttle and the crate aboard their carriers, and the forced checkpoint counts all three persistent entities; a part is busy with a transient drone or a client's pawn aboard and idle with only saved vehicles). The two-worker leg is `Tests/EditMode/ConformanceGhostCarrierRidersTests.cs` (3 tests, Unity only: a ship at the seam, ghosted to a second worker that owns a crate and a drone aboard through a pinned interior, leaves with both when its owner empties the yard, nothing is set down there and the crate's record names the ship's own key; despawned for good, both riders are put down in the yard, still owned by the second worker; the despawn's two trailing fields are written only when the riders leave). `Tests/EditMode/ConformanceChunkReleaseTests.cs` (NEB-262, 1 test: a chunk is kept while an entity filed in it is only a ghost here, is released with the entity checkpointed once its authority arrives, and the entity is restored exactly once when the chunk is leased again). Design: `docs/dynamic-worlds.md`, "Retiring a chunk under a carrier" (D4 for the two-worker case). | covered |
| 19 | A worker declared dead while it is still running (its heartbeats lost, not its process) leaves exactly one live copy of each persistent entity, holding the survivor's state: it fences itself on the same liveness test the orchestrator applies and saves, restores and hands over nothing while fenced; the survivor's restore stamps the new epoch into the store first, so a save the old owner had queued is refused; the old owner, on learning the verdict, drops what it lost without a save; peers and gateways drop a declared-dead worker even while its link is up, so no client is shown the old copy beside the restored one; a fence that was a false alarm loses nothing. | NEB-256 | B (two real workers on `ConformanceMesh`, a real `NebulaPersistence` each over one shared `LocalPersistenceStore`, a `LocalControlPlane` with a driven clock) + A (the fence and the roster over `LocalControlPlane`; the gateway over loopback with a silenced `FakeWorker`) | `Tests/EditMode/ConformanceDeclaredDeadWorkerTests.cs` (3 tests, Unity only: the full scenario with three dirty crates, the late save refused and exactly one copy left on the survivor; a fence lifted by a heartbeat saves what changed meanwhile; a fenced worker hands nothing over until its heartbeat lands) and `Services~/Nebula.Services.Tests/DeclaredDeadWorkerTests.cs` (6 tests: the fence agrees with the orchestrator's test, a same-document unregister is a verdict and a replacement document is not, a worker that has not seen its row is not fenced, the roster reports a removal once, a gateway drops a declared-dead worker whose link is up and the client never holds both copies). Design: `docs/persistence-durability.md` D7–D12. | covered |
