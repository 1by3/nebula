...
Control plane + persistence — hosted by the orchestrator and kept in SQLite or PostgreSQL (SpacetimeDB was the first backend and was removed in favor of the orchestrator hosting the state itself). Two jobs: (1) the control plane — container ownership leases, authority epochs, node registry, entity directory; low write volume, transactionally consistent, subscription push. A lease row is roughly container_id, owner_node, epoch, state, heartbeat_at, expires_at with state ∈ {assigning, active, draining, orphaned}. (2) Durable world state — inventories, placed structures, player persistence — written on checkpoint interval and container drain, read on hydration. What it must never be: the transport for per-tick transforms or the arbiter of hit registration. All sim code talks to an IControlPlane abstraction; the orchestrator hosts the real one and workers mirror it over HTTP. The honest risk is the single orchestrator (no failover yet); its state is small and durable, so a standby reading the same PostgreSQL database is the obvious next step. Control plane and persistence share a database but not a table, since their consistency and durability needs diverge.
...
Any number of gateways serve one mesh, behind a load balancer that Nebula does not provide. The pieces that make that safe (protocol 15, September 2026):
...
One connection per player (NebulaConfig.SingleSessionPerPlayer, default on). A player has one session. On welcome the gateway looks for a session this identity already has: the session token, then another welcomed client of its own with the same Identity, then any entity in _entities whose OwnerIdentity matches (every worker announces every entity to every gateway, so this also finds a pawn welcomed elsewhere or one being held through the reclaim grace). It takes that session id over with a fresh generation and lets the old link go with SessionReplaced, which the client shows and then stays out on - it clears its session token and stops reconnecting, or the two connections would take the player from each other in turn. Cross-gateway, the worker closes the loop: a Claim.Reclaimed from a different gateway makes it send EndSession to the gateway that lost the session (fenced by generation, and the gateway does not despawn the pawn on it), and two sessions for one identity - only possible when two gateways welcome the same player in the same instant - are resolved on the worker by generation, the older one losing its pawn or pending spawn. Off, every connection gets its own session and pawn (a bot fleet sharing one account; nebula-loadgen --token).
