...
Any number of gateways serve one mesh, behind a load balancer that Nebula does not provide. The pieces that make that safe (protocol 16, September 2026):
...
One connection per player (NebulaConfig.SingleSessionPerPlayer, default on) is enforced at gateway admission. The orchestrator serializes per-identity claims, retains the session ID and pending spawn assignment, and increments connection generations. A replacement is not welcomed until the previous gateway acknowledges disconnection. If the previous gateway cannot acknowledge or the coordinator is unavailable, the new join is rejected; existing connections continue. File and SQL stores retain each claim before acknowledgment, separately from broadcast topology snapshots. Workers retain generation fencing but no longer scan local player identities to resolve duplicates. Disable the setting consistently on every gateway to allow independent sessions per identity. After a gateway crash, recovery may require stopping every gateway and explicitly clearing claims with nebula start --reset-sessions.
