...
Installs what is missing and reports what it finds:
  dotnet      the .NET 10 SDK the standalone orchestrator and gateway are built with, via dotnet-install into ~/.dotnet
...
";
    ...
    public override int Run(Context ctx, ParsedArgs args)
    {
        ...
        Ui.Step(".NET SDK (the orchestrator and gateway need 10.0 or newer)");
    ...
    private static void InstallDotnet()
    {
        ...
        Ui.Info("the SDK is in ~/.dotnet; the CLI finds it there for service builds. Add it to your own PATH to use `dotnet` directly.");
    }
